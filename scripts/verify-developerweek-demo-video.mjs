...
const outDir =
  process.env.AGENTGUARD_DEVELOPERWEEK_VIDEO_DIR ?? join(root, "agentguard-runs", "developerweek-demo-video");
const minSeconds = Number(process.env.AGENTGUARD_DEVELOPERWEEK_VIDEO_MIN_SECONDS ?? 90);
const maxSeconds = Number(process.env.AGENTGUARD_DEVELOPERWEEK_VIDEO_MAX_SECONDS ?? 180);
const minWidth = Number(process.env.AGENTGUARD_DEVELOPERWEEK_VIDEO_MIN_WIDTH ?? 1280);
const minHeight = Number(process.env.AGENTGUARD_DEVELOPERWEEK_VIDEO_MIN_HEIGHT ?? 720);
const minBytes = Number(process.env.AGENTGUARD_DEVELOPERWEEK_VIDEO_MIN_BYTES ?? 10_000);
const syncToleranceSeconds = Number(process.env.AGENTGUARD_DEVELOPERWEEK_VIDEO_SYNC_TOLERANCE_SECONDS ?? 6);
const maxSegmentPaddingSeconds = Number(process.env.AGENTGUARD_DEVELOPERWEEK_AUDIO_MAX_SEGMENT_PADDING_SECONDS ?? 3);
const maxDetectedSilenceSeconds = Number(process.env.AGENTGUARD_DEVELOPERWEEK_AUDIO_MAX_DETECTED_SILENCE_SECONDS ?? 3.5);
let inspectedVideoDurationSeconds = null;

const paths = {
  mp4: join(outDir, "AgentGuard-CI-DeveloperWeek-Demo.mp4"),
  ...
  demoManifest: join(outDir, "AgentGuard-CI-DeveloperWeek-Demo.manifest.json")
};
...
function checkShotListAndAudio() {
  ...
  const hasOnlyAllowedRoutes = shotList.every(
    (shot) =>
      ...
      (shot.url.includes("?contest=developerweek") &&
  ...
  if (shotList.length === 6 && totalSeconds <= maxSeconds && hasTerminalScene && hasOnlyAllowedRoutes) {
    pass(
      "shot-list:routes",
      `${shotList.length} scenes / ${totalSeconds}s / terminal plus DeveloperWeek product routes`
    );
  } else {
    fail(
      "shot-list:routes",
      `Expected 6 DeveloperWeek scenes under ${maxSeconds}s with one terminal scene. Scenes: ${shotList.length}; seconds: ${totalSeconds}; terminal: ${hasTerminalScene}; allowed routes: ${hasOnlyAllowedRoutes}.`
    );
  }
...
function checkNarration() {
  ...
  const missingPhrases = requiredPhrases.filter((phrase) => !generatedSpeech.includes(phrase));

  if (redFlags.length === 0 && missingPhrases.length === 0 && voiceover.trim().length > 500) {
    pass("narration:clean", "DeveloperWeek-specific narration with no internal markers");
  } else {
    fail(
      "narration:clean",
      `Narration must be DeveloperWeek-specific and free of internal markers. Red flags: ${
        redFlags.join(", ") || "none"
      }; missing phrases: ${missingPhrases.join(", ") || "none"}; length: ${voiceover.trim().length}.`
    );
  }
}
...
if (failures.length > 0) {
  console.error(`\nDeveloperWeek demo video check failed: ${failures.length} issue(s).`);
  process.exit(1);
}

console.log(`\nDeveloperWeek demo video check passed: ${checks.length} checks.`);
