...
const canonicalWavPath = join(outDir, "AgentGuard-CI-Splunk-Voiceover-en.wav");
const prepareOnly = process.argv.includes("--prepare-only");

const edgeVoice = process.env.AGENTGUARD_SPLUNK_EDGE_VOICE ?? "en-US-EmmaMultilingualNeural";
const edgeRate = process.env.AGENTGUARD_SPLUNK_EDGE_RATE ?? "+12%";
const edgePitch = process.env.AGENTGUARD_SPLUNK_EDGE_PITCH ?? "+0Hz";
...
function assertCleanNarration(text) {
  const forbiddenTokens = ["undefined", "NaN", "file://", "##", "Screen narration", "in the hackathon scene"];
...
function writeReviewFile(shotList) {
  const lines = [
    "# AgentGuard CI Splunk Demo Voiceover Review",
    "",
...
function synthesizeSegment(shot, index) {
  ...
  if (spokenSeconds > targetSeconds - 0.5) {
    const tempo = Math.min(1.65, Math.max(1.02, spokenSeconds / Math.max(1, targetSeconds - 0.9)));
    run(ffmpeg.path, ["-y", "-i", rawWavPath, "-filter:a", `atempo=${tempo.toFixed(3)}`, "-ar", "44100", "-ac", "2", fittedWavPath]);
...
requireFile(shotListPath, "Splunk shot list. Run npm run video:prep:splunk first");
...
if (prepareOnly) {
  ...
  console.log("Splunk demo voiceover text prepared.");
...
console.log("Splunk demo voiceover audio generated.");
