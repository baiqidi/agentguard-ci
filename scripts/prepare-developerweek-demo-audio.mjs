...
const canonicalWavPath = join(outDir, "AgentGuard-CI-DeveloperWeek-Voiceover-en.wav");
const prepareOnly = process.argv.includes("--prepare-only");

const edgeVoice = process.env.AGENTGUARD_DEVELOPERWEEK_EDGE_VOICE ?? "en-US-EmmaMultilingualNeural";
const edgeRate = process.env.AGENTGUARD_DEVELOPERWEEK_EDGE_RATE ?? "+18%";
const edgePitch = process.env.AGENTGUARD_DEVELOPERWEEK_EDGE_PITCH ?? "+0Hz";
...
function assertCleanNarration(text) {
  ...
  if (leakedToken) {
    throw new Error(`Voiceover script contains non-spoken or off-target marker: ${leakedToken}`);
  }
}
...
function writeReviewFile(shotList) {
  const lines = [
    "# AgentGuard CI DeveloperWeek Demo Voiceover Review",
    "",
...
function synthesizeSegment(shot, index) {
  ...
  if (spokenSeconds > targetSeconds - 0.4) {
    const tempo = Math.min(1.85, Math.max(1.03, spokenSeconds / Math.max(1, targetSeconds - 0.7)));
    run(ffmpeg.path, [
      "-y",
      "-i",
      rawWavPath,
...
requireFile(shotListPath, "DeveloperWeek shot list. Run npm run video:prep:developerweek first");
...
if (prepareOnly) {
  ...
  console.log("DeveloperWeek demo voiceover text prepared.");
...
console.log("DeveloperWeek demo voiceover audio generated.");
