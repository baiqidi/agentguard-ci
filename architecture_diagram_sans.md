...
flowchart LR
    ...
    subgraph SIFT["SANS SIFT Workstation"]
        Tools["fls, mactime, rip.pl, grep, awk, tshark"]
...
1. Evidence sources enter a SIFT-compatible analysis path: disk timeline, registry export, authentication log, and network flow index.
...
4. The output pipeline writes `agent-execution-log.jsonl`, `accuracy-report.json`, `evidence-dataset.md`, `investigative-narrative.md`, and SANS-mode `sift-ir-evidence.json` packets.
...
- **Breadth and depth:** disk persistence, authentication-log accuracy, and containment approval routes.
